...
export interface ModelConfig {
  ...
  display_name: string;
  /** Exact string the Lava API expects in the `model` field. */
   ...
   * Array because some models (Claude, GPT) run on multiple clouds.
   */
...
export const MODELS: ModelConfig[] = [
  ...
  // ── Capable / complex tasks (difficulty 4–15) ─────────────────────────────
  {
    ...
    difficulty_min: 4,
    difficulty_max: 10,
    flops_per_token: 100,      // MoE ~50B active, 2×50 = 100 GFLOPs (light think)
    fulfillment_companies: ["Google Cloud"],
  },
  {
  ...
  },
  {
    ...
    difficulty_min: 7,
    difficulty_max: 15,
    flops_per_token: 200,      // MoE ~100B active, 2×100 = 200 GFLOPs (medium reasoning)
    fulfillment_companies: ["Azure", "Oracle", "SoftBank", "Nvidia"],
  },
  {
    model_id: "o4-mini",
 ...
 *
 * Falls back to the highest-flops model if no range matches (out-of-bounds score).
 */
...
/** Lookup map from model_id → flops_per_token. Used by carbon.ts. */
